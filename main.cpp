...
enum SYSTEM {SIMPLE, PENDULUM, CHAIN, CLOTH};
...
namespace
{
    ...
    void initSystem(SYSTEM sysType, INTEGRATOR intType)
    {
        ...
        switch(sysType) {
            case SIMPLE:
                system = new SimpleSystem(intType, 250);
                break;
	...
	//Draw control particle(s)
	for (int ii = 0; ii < system->controlParticles.size(); ii++) {
	    Vector3f pos = system->controlParticles[ii].position;
	    glPushMatrix();
	...
	case 'o':
	{
	    cout << "turn on/off opengazer" << endl;
	    break;
	}
	case 'c':
	{
	    cout << "turn on/off control particles" << endl;;
	    break;
	}
        case 's': // shake!
        {
...
int main( int argc, char* argv[] )
{
    ...
    SYSTEM system_type = SIMPLE; // default
    INTEGRATOR integrator_type = GAZER; //default
